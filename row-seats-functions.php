...
//Displaying transaction details on admin liftOff
					if(isset($_GET['action']) && $_GET['action']=="rst-transaction-details")
					{
						if (isset($_GET["id"]) && !empty($_GET["id"])) {
							...
							if (intval($transaction_details["id"]) != 0) {
								...
								<tr>
									...
									<td>'.esc_attr(urldecode($data1)).'</td>
								</tr>';
								}
								echo '
								</table>
...
//Deleting a transaction from admin side		
					if (isset($_GET['action']) && $_GET['action']=="rst-delete-transaction")
					{
						$id = intval($_GET["id"]);
						$transaction_details = $wpdb->get_row("SELECT * FROM rst_payment_transactions WHERE id = '".$id."' AND deleted = '0'", ARRAY_A);
...
//Setting an offline registration as paid from admin side		
					if( isset($_GET['action']) && $_GET['action']=="rst-paid-transaction")
					{
						$id = intval($_GET["id"]);
						$transaction_details = $wpdb->get_row("SELECT * FROM rst_payment_transactions WHERE id = '".$id."' AND deleted = '0'", ARRAY_A);
						if (intval($transaction_details["id"]) != 0) {	
        ...
        and bsr.booking_id =" . $transaction_details["tx_str"];		
					...
					if(isset($_GET['action']) && $_GET['action']=="rst-export-transactions")
					{
...
//print "<br><br><br>";
	...
	$myitems=array();
			...
			for ($i = 0; $i < count($cartitems); $i++) {
				...
				{
					$myproductsarraytemp=explode("#",$_POST['special_pricing'.$i]);
...
$data['myitems']=$myitems;			
		$data['event_name_display']=$eventname1." ".implode(",",$mytseats);
			...
			//Amenities addons -START
			if($_POST['totalamenities'] && $_POST['totalamenities']>0)	
			{
				
				$amenitiesarray=array();
				
				for($am=0;$am<$_POST['totalamenities'];$am++)
                {
					
					
				$amenityname=$_POST["aminitylabel".$am];
                $amenityprice=$_POST["aminityprice".$am];
				$amenityqty=$_POST["aminityqty".$am];
		
                $amenitytotal=$amenityprice*$amenityqty;

                 if($amenityqty>0)
                 {					 
					
				echo '	<tr>
						<td class="row_seats__confirmation_title">'.$amenityname.' ('.$symbol.number_format( $amenityprice, 2, ".", "").'*'.$amenityqty.'):</td>
						<td class="row_seats__confirmation_data">'.$symbol.number_format( $amenitytotal, 2, ".", "").'</td>
					</tr>';
				$amenitiesarray[] = array(
								"label" => $amenityname,
								"price" => $amenityprice,
								"quantity" => $amenityqty,
								"pricetotal" => $amenitytotal
							);	
				 }
					
					
				}
				$data['amenities']=base64_encode(serialize($amenitiesarray));
				$_SESSION['amenities']=base64_encode(serialize($amenitiesarray));

			}
			//Amenities addons -END			
...
// PCoder -- date issue -- changing the date to 21/08/2015, makes the event end.

// strtotime() does not work with format 'd/m/Y' [see http://stackoverflow.com/a/15379246/782094]
// so check if the option set is d/m/Y .. in that case change the option to d-m-Y 
if(get_option('date_format') == 'd/m/Y'){
	$eventdate = date('d-m-Y', strtotime($daten)). ' ' .date(get_option('time_format'), strtotime($timen));
} else if(strpos(get_option('date_format'), 'F') || strpos(get_option('date_format'), 'm')|| strpos(get_option('date_format'), 'D')|| strpos(get_option('date_format'), 'l')){
	// PCoder - i18n issue : Dates such as 21 diciembre, 2015 are not able to be parsed by strtotime properly
	// Hence, we convert them to d-m-Y format like above, and revert it back later
	$eventdate = date('d-m-Y', strtotime($daten)). ' ' .date(get_option('time_format'), strtotime($timen));
}
...
echo '<div><br/><strong>'.$rstidlemsg.'</strong></div></div><br>';
	...
	echo   '<div><br/><strong>'.$wpuser_onlyloggedin.'</strong></div></div>';
	  ...
	  //  elseif ($currentdate >= $eventdate1) {
		//enable seat booking for admin even if the event is expired
	  elseif (($currentdate >= $eventdate1)  && (apply_filters('row_seat_offline_reservation_show_expired_events',true)==true)) {
    ...
    switch ($action) {
        ...
        case 'savebooking':
			...
			if($rst_options['rst_enable_special_pricing']=="on" and row_seats_special_pricing_verification())
			{
					...
					$myproductsarray=explode("__",$myproducts);
					for ($i = 0; $i < count($myproductsarray); $i++) {
						$myproductsarraytemp=explode("#",$myproductsarray[$i]);
		...
		//Amenties- below code will store amenities selected by customer in booking details base64encoded and sealized -START
		$amenities_query= apply_filters('row_seats_amenties_query',$data['bookingid']);	
		//Amenties- below code will store amenities selected by customer in booking details base64encoded and sealized -End		
	    ...
	    return $data['bookingid'];
			...
			//Amenties- below code will store amenities selected by customer in booking details base64encoded and sealized  _START
			$amenities_query= apply_filters('row_seats_amenties_query',$wpdb->insert_id);	
			//Amenties- below code will store amenities selected by customer in booking details base64encoded and sealized   -END			
...
{
global $screenspacing,$wpdb, $seats_avail_per_row, $k;
    $rst_paypal_options = get_option(RSTPLN_PPOPTIONS);
    $symbol = $rst_paypal_options['currencysymbol'];
	$symbol = get_option('rst_currencysymbol');
    $symbols = array(
...
//print_r($seats);
    for ($i = 0; $i < count($seats); $i++) {
        ...
        $seatdiscost = $data['discount_price'];
        $hide_row = "";
        if(isset($rst_options['row_seats_table_hiderow_'.$showid]))
			$hide_row = $rst_options['row_seats_table_hiderow_'.$showid];
        if ($i == 0) {
            if ($rowname == '') {
            	if($hide_row == 'on'){
        ...
        else if ($data['seattype'] == 'S' && isset($stall[$seatno]))
            $html .= '<li class="s showseats" id="' . $showname . '' . $showid . $rowname . $seatno . '" title="" rel="">' . $stall[$seatno] . '</li>';
        else if ($data['seattype'] == 'L' && isset($balcony[$seatno]))
            $html .= '<li class="l showseats" id="' . $showname . '' . $showid . $rowname . $seatno . '" title="" rel="">' . $balcony[$seatno] . '</li>';
        else if ($data['seattype'] == 'C' && isset($circle[$seatno]))
            $html .= '<li class="c showseats" id="' . $showname . '' . $showid . $rowname . $seatno . '" title="" rel="">' . $circle[$seatno] . '</li>';
        else
    ...
    $symbol = $symbols[$symbol];
		...
		$payer_offlinepayment	=$booking_details[0]['email'];
			...
			$wpdb->query($sql) or die(mysql_error());*/
								...
								$ret = $wpdb->insert( 
								"rst_payment_transactions", 
								array( 'tx_str' => $inid, 
									   'payer_name' => $payer_name,
									   'payer_email' => $payer_offlinepayment,
									   'gross' => floatval($gross_total),
									   'currency' => $mc_currency,
									   'payment_status' =>$payment_status ,
									   'transaction_type' =>'Offline Payment:'.$transaction_type ,
									   'details' => addslashes($paymentData),
									   'created' => time(),
									   'deleted' => '0',
								...
								array('%s', '%s', '%s', '%f', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s') 
							);		
